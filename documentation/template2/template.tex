...
\usepackage[backend=bibtex,style=authoryear,natbib=true]{biblatex} % Use the bibtex backend with the authoryear citation style (which resembles APA)
	$if(biblio-files)$
	$for(biblio-files)$
		\addbibresource{$biblio-files$} % The filename of the bibliography
	$endfor$
	$endif$
$endif$
...
$if(author)$
	...
	\author{\authorstyle{$for(author)$$author$$sep$$endfor$}} % Authors	
	%\newline\newline % Space before institutions
	%$for(institute)$\institution{$institute$}$endfor$
$endif$
...
$if(biblatex)$
	$if(biblio-files)$
		...
		$if(biblio-title)$
			\printbibliography[title={$biblio-title$}] % Print the bibliography, section title in curly brackets
		$else$
